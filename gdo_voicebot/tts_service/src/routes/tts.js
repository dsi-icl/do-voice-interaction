...
export async function textToSpeech (req, res) {
  try {
    ...
    // if (true) {
    const data = speakCereProc(req.query.text)
    if (data === '') {
      res.status(400).json({ status: 'fail', service: 'Text To Speech service', error: 'Data is empty' })
    }
...
function textToWav (text, output) {
  ...
  const cerevoiceEng = require('./cereproc/cereproc_lib/cerevoice_eng/jslib/cerevoice_eng.node')
  // Load the voice using the full API
  var eng = cerevoiceEng.CPRCEN_engine_load(config.voice_file, config.licence_file, config.root_certificate, config.client_certificate, config.client_key)

  var chan = cerevoiceEng.CPRCEN_engine_open_default_channel(eng)

  var len = 0
  if (text) {
    len = text.length
  }
  var abuf = cerevoiceEng.CPRCEN_engine_channel_speak(eng, chan, text, len, 1)

  return cerevoiceEng.CPRC_abuf_wav_data(abuf)
